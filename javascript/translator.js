...
function encodeToBraille(input) {
    ...
    chars.forEach((char, index) => {
        if (/[A-Z]/.test(char)) {
            output += brailleAlphabet['capital']; // Add capital indicator
            char = char.toLowerCase();
        } else if (/\d/.test(char)) {
            if (!numberMode) {
                output += brailleAlphabet['number']; // Add number indicator
                numberMode = true;
            }
        } else {
            numberMode = false; // Reset number mode on non-number character
        }

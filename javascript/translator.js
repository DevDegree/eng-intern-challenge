...
const BRAILLE_REVERSE_MAP = Object.fromEntries(
...
function translateToEnglish(brailleBinary) {
    const translated = [];
    let index = 0;
    let numberMode = false;
...
function binaryToBrailleDots(binary) {
    return Array.from({ length: 6 }, (_, i) => (binary & (1 << (5 - i))) ? 'O' : '.').join('');
}

function brailleDotsToBinary(dots) {

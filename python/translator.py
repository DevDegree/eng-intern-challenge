...
def english_to_braille(text: str) -> str:
    ...
    for word in words:
        ...
        for c in word:
            braille_word.append(get_braille(c, is_number=is_number))
        braille_words.append("".join(braille_word))

    return get_braille(" ").join(braille_words)
...
def braille_to_english(text: str) -> str:
    ...
    for i in range(0, len(text), BRAILLE_LENGTH):
        ...
        if braille == BrailleMapping.CAPITAL_FOLLOWS:
            is_caps = True
            continue
        if braille == get_braille(" "):
            is_number = False

        character = get_english(braille, is_number=is_number, is_caps=is_caps)
...
def main():
    ...
    if is_braille(input):
        translated = braille_to_english(input)
    else:
        translated = english_to_braille(input)

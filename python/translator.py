...
# Maping lowercase letters and space to braille
map_braille = {
...
def english_to_braille(text):
    ...
    for char in text:
        if char.isdigit():
            if not num:
                result.append(indicator_number)
                num = True
            result.append(map_numbers[char])
        elif char.isalpha():
            if num:
                num = False 
            if char.isupper():
                result.append(indicator_capital)
...
def braille_to_english(braille):
    ...
    while i < len(braille):
        ...
        if symbol == indicator_capital:
            cap = True
        elif symbol == indicator_number:
            num = True
        else:
            if num:

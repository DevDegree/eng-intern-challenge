...
def main():
    ...
    if (re.search("^[.O]*$", str)):
        ...
        reverse_hash = dict((v, k) for k, v in hash.items())

        capitalization = False

        for substring in len_six_substr:
            if substring in reverse_hash.keys():
                if capitalization:
                    result_string += chr(reverse_hash[substring] - 32)
                    capitalization = False
                elif reverse_hash[substring] == "capital":
                    capitalization = True
                elif reverse_hash[substring] != 'number':
                    result_string += chr(reverse_hash[substring])

    else:
        ascii_str = [ord(c) for c in str]

        for ascii in ascii_str:
            if (ascii > 47 and ascii < 58 and not num_sequence):

# Some credits to LazoCoder on GitHub
...
def word_to_alpha(word):
    #convert a braille word to alphabet based text - handles contractions and capitalization
    
    # Check if the word is a known Braille contraction
    if word in mapBrailleToAlpha.contractions:
        ...
        for i in range(len(word)):
            if word[i] == CAPITAL:
                # if the current character is the CAPITAL marker, set the flag for the next character
                capitalize_next = True
                continue  # Skip to the next character
            
            if word[i] in mapBrailleToAlpha.letters:
                # convert braille letter to english letter
                letter = mapBrailleToAlpha.letters.get(word[i])
                if capitalize_next:
                    # if the capitalize flag is set - convert the letter to uppercase
                    letter = letter.upper()
                    capitalize_next = False  # reset the flag
                result += letter
            elif word[i] in mapBrailleToAlpha.punctuation:
                # convert braille punctuation to english punctuation
...
def translate_braille_to_alpha(string):

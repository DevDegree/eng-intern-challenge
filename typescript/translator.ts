// establish translation reference
const translationConfig: { [key:string]: {[key:string]: string} } = {
  modifiers: {
    "capital follows": ".....O",
...
// establish regex to identify English, letters or numbers
...
function isItBraile() {
  ...
  if (notBraileExp.test(input[0])) return false;
  ...
  if (input[0].length % 6 != 0) return false;
...
function translateBraileToEnglish() {
  ...
  // parse each character into english, allowing for "x follows" modifiers
    ...
    if (char == translationConfig.letters[" "]) {
      isNumberFollows = false;
      english += " ";
      return;
    }
    // if it's "number follows", set isNumberFollows to true
    if (char == translationConfig.modifiers["number follows"]) isNumberFollows = true;
    // if it's "capital follows", set isCapitalFollows to true
    if (char == translationConfig.modifiers["capital follows"]) isCapitalFollows = true;
    // if isNumberFollows is true, translate from number definitions
    if (isNumberFollows) {
      for(let n in translationConfig.numbers) if (char == translationConfig.numbers[n]) english += n;
    } else {
      // otherwise, translate from letter definitions
      for (let l in translationConfig.letters) {
        if (char == translationConfig.letters[l]) {
          // use a capital letter as indicated
          if (isCapitalFollows) {
            english += l.toUpperCase();
            // and reset isCapitalFollows to false
...
function translateEnglishToBraile() {
  ...
  characters.forEach((character) => {
    // if character is a space, turn off isNumberFollows
    if (character == " ") isNumberFollows = false;
    // if character is a number, and isNumberFollows is false add "numbers follows";
    if (numberExp.test(character)) {
      ...
      if (capitalExp.test(character)) braile += translationConfig.modifiers["capital follows"];
      // parse the character into braile
      if (translationConfig.letters[character.toLowerCase()] !== undefined) braile += translationConfig.letters[character.toLowerCase()];
      /* stretch goal: add a flag or notification for undefined characters (e.g. punctuation) */
    }
  })
  return braile;
}

if (isItBraile()) output = translateBraileToEnglish();
else output = translateEnglishToBraile();

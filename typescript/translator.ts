...
const translateToBraille = (input: string): string => {
  ...
  for (const char of input) {
    ...
    if (/[0-9]/.test(char)) {
      // ensure number sign has been added & append number
      if (!isNumber) {
        result += numberSign;
        isNumber = true;
      }
      ...
      // ensure capital sign has been added if necessary & append char
      isNumber = false;
...
const translateToEnglish = (input: string): string => {
  ...
  for (let i = 0; i < input.length; i += 6) {
    ...
    if (brailleChar === capitalSign) {
      ...
      // handle braille after number sign
      ...
      // handle braille representing alphabetic letters
